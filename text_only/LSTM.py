...
class LSTM(pl.LightningModule):
    def __init__(self, word_vectors, wv_dim, hidden_size, max_seq_len, output_size):
        # feature_size 就 128？
        super(LSTM, self).__init__()
        self.embedding = nn.Embedding.from_pretrained(torch.FloatTensor(word_vectors.vectors), freeze=False)
        # self.lstm = nn.LSTM(input_size=wv_dim, hidden_size=hidden_size, num_layers=1)
        self.lstm = nn.LSTM(input_size=wv_dim, hidden_size=hidden_size, bidirectional=False)
        self.decoder = nn.Linear(in_features=hidden_size, out_features=output_size)
        self.dropout = nn.Dropout(0.5)

    def forward(self, x):
        x = self.embedding(x)  # [batch, seq_len, 128]
        x = x.permute(1, 0, 2)  # [17, batch, 770]
        lstm_output, (hn, cn) = self.lstm(x)
        hn = hn.squeeze()
        dscp_hidden = self.dropout(hn)
        y = self.decoder(dscp_hidden)
        return y
...
word_vectors = gensim.models.KeyedVectors.load_word2vec_format('data/pw_word_vectors.txt', binary=False)
text_all = np.array([line.strip().split(' =->= ')[1]
                     for line in open('data/dscps_encoded.txt', 'r', encoding='utf8').readlines()])
tags_all = np.array(
    [int(line.strip()) for line in open('data/tags_id.txt', 'r', encoding='utf8').readlines()])
...
model = LSTM(word_vectors, 128, 150, SEQ_MAX_LEN, NUM_CLASS)
