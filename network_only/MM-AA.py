...
class MMAADataset(torch.utils.data.Dataset):
...
class MMAAGraphClassifier(pl.LightningModule):
...
train_dataset = MMAADataset(train_Y, train_nodes)

val_Y = tags_all[val_ids]
val_dataset = MMAADataset(val_Y, val_nodes)

test_Y = tags_all[test_ids]
test_dataset = MMAADataset(test_Y, test_nodes)
...
model = MMAAGraphClassifier(CLASSNUM, node_embeddings.vectors, node_embeddings.vector_size)
trainer = pl.Trainer(max_epochs=50, gpus='0', callbacks=[EarlyStopping(monitor='val_loss', patience=5)])
