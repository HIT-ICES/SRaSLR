...
class ALLCatClassifier(pl.LightningModule):
    def __init__(self, classes, node_embedding, node_embedding2, node_embedding_size, dense_dropout=0.5, **kwargs):
        super().__init__()
        # Load model
        ...
        self.decoder = nn.Sequential(
            ...
            nn.Linear(in_features=2 * node_embedding_size, out_features=2 * node_embedding_size),
            nn.Dropout(dense_dropout),
            nn.Linear(in_features=2 * node_embedding_size, out_features=classes),
        )

    def forward(self, node_ids1, node_ids2):
        node_output = self.node_encoder(node_ids1)
        node_output2 = self.node_encoder2(node_ids2)
        output = torch.cat((node_output, node_output2), 1)
        output = self.decoder(output)
        return output

    def configure_optimizers(self):
        bert_params = list(map(id, self.node_encoder2.parameters()))
...
all_model = ALLCatClassifier(CLASSNUM, node_embeddings.vectors, node_embeddings2.vectors,
...
trainer.fit(all_model, train_dl, val_dl)
trainer.test(all_model, test_dl)
